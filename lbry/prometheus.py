...
probe_in_flight = Counter("probes_in_flight", "Number of loop probes in flight", namespace='asyncio')
probe_finished = Counter("probes_finished", "Number of finished loop probes", namespace='asyncio')
...
class PrometheusServer:
    ...
    async def start(self, interface: str, port: int):
        ...
        self.logger.info('prometheus metrics server listening on %s:%i', *metrics_site._server.sockets[0].getsockname()[:2])
